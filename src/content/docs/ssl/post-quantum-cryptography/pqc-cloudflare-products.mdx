...
This section only covers the inbound TLS connection from the end-user client to Cloudflare's edge. When a Worker fetches data from a backend storage service ([D1](/d1/), [KV](/kv/), [Durable Objects](/durable-objects/), [R2](/r2/), [Workers AI](/workers-ai/), [Hyperdrive](/hyperdrive/), and similar), that connection is governed by the [Cloudflare internal network](#cloudflare-internal-network) section. When a Worker calls out to a third-party origin via `fetch()`, it is governed by the [Cloudflare to origin](#cloudflare-to-origin) section. The [Agentless on-ramp (PAC files)](#agentless-on-ramp-pac-files) for Cloudflare Gateway terminates inbound TLS in its own edge stack and is covered separately below.
...
### Agentless on-ramp (PAC files)

Cloudflare Gateway proxy endpoints let browsers reach Cloudflare's network over TLS 1.3 without an agent installed on the device. Browsers are configured via a Proxy Auto-Configuration (PAC) file or system proxy settings to forward traffic to a Cloudflare-hosted proxy endpoint, which terminates TLS at Cloudflare's edge.
...
[Cloudflare Gateway](/cloudflare-one/traffic-policies/http-policies/tls-decryption/#post-quantum-support) is a Secure Web Gateway that runs on Cloudflare's edge and filters HTTPS traffic. Gateway has no client-side component; clients reach Gateway via one of several post-quantum on-ramps:

- The [Cloudflare One Client](#cloudflare-one-client).
- A [Cloudflare IPsec](#cloudflare-ipsec) tunnel.
- The [Agentless on-ramp (PAC files)](#agentless-on-ramp-pac-files).
